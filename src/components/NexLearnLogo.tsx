...
export default function NexLearnLogo({
  ...
  const dimensions = {
    sm: { icon: 32, text: '1.15rem', gap: 10, badge: '0.65rem' },
    md: { icon: 50, text: '1.65rem', gap: 12, badge: '0.78rem' },
    lg: { icon: 56, text: '1.9rem', gap: 14, badge: '0.82rem' },
    xl: { icon: 72, text: '2.5rem', gap: 18, badge: '0.9rem' },
  }[size];

  const effectiveTextSize = textSize || dimensions.text;
  const effectiveBadgeSize = badgeSize || dimensions.badge;
    ...
    >
      ...
      {showText && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0, whiteSpace: 'nowrap' }}>
          ...
          >
            <span style={{ color: 'var(--lp-text-primary, var(--text))', fontSize: 'inherit', textTransform: 'none', fontWeight: 'inherit', letterSpacing: 'inherit' }}>
              Nex
            </span>
            <span
              style={{
