  ...
  const downloadReport = async (isForEmail = false) => {
    ...
    try {
      ...
      const addHeader = (doc, pageNum) => {
        doc.setFillColor(79, 70, 229); // ICM System Primary
        doc.rect(0, 0, pdfWidth, 22, 'F');
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(18);
        doc.setFont('helvetica', 'bold');
        doc.text('ICM System ', margin, 14);
        doc.setFontSize(9);
      ...
      const addFooter = (doc) => {
        ...
        doc.text('This is an official ICM System academic record. Information is verified through blockchain-backed credentials.', margin, footerY);
      ...
      const fileName = `ICM_System_Official_Report_${new Date().getFullYear()}.pdf`;
      if (isForEmail) {
    ...
    <div>
      ...
      <div className="print-only-header" style={{ display: 'none' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0', borderBottom: '2px solid var(--primary)' }}>
          <div>
            <h1 style={{ color: 'var(--primary)', margin: 0 }}>ICM System Analytics</h1>
      ...
      <div className="print-only-header" style={{ display: 'none' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0', borderBottom: '2px solid var(--primary)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h1 style={{ color: 'var(--primary)', margin: 0, fontSize: '1.5rem' }}>ICM System Academic Record</h1>
              <p style={{ color: 'var(--text-secondary)', margin: 0, fontSize: '0.8rem' }}>Intelligent Course Management System</p>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
