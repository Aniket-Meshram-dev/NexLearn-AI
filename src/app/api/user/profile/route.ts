...
export async function GET() {
  ...
  courses.forEach(c => recentActivities.push({ title: `Started ${c.title}`, date: c.createdAt, icon: 'book-open' }));
  ...
  return NextResponse.json({ 
    ...
    recentActivity: recentActivities.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 4)
  });
}
...
export async function PUT(request) {
  ...
  const data: any = { name, learningGoal, gender, bio, country, state, city, educationLevel };
  if (dob) {
